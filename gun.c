...
pistol* pistol_create(){																				//Implementação da função "pistol_create" (!)
	...
	new_pistol->bullet_art = NULL;																		//Inicializa a lista de disparos com NULL; ou seja, sem disparos (!)
...
void pistol_destroy(pistol *element){																	//Implementação da função "pistol_destroy" (!)
	...
	if(element->bullet_art)																				//Se a arte da pistola não for NULL (!)
		al_destroy_bitmap(element->bullet_art);
