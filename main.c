...
#define NUM_PLATFORMS 4 // Define the movement speed of bullets
...
int main(){
    ...
    create_map(vector_platform, SCREEN_X, SCREEN_Y, NUM_PLATFORMS, all_background, sprite_floor); // Create the map
            ...
            case ALLEGRO_EVENT_TIMER:
                if(tela == MENU){
                    ...
                    if(vector_platform[3]->x - map_ajustment < SCREEN_X && vector_platform[3]->x - map_ajustment + vector_platform[3]->width > 0) {
                        //draw a short platform
                            for(int i = 1; i < 5; i++) { // Draw a platform
                                al_draw_tinted_scaled_rotated_bitmap_region(sprite_floor, 0, 64, 64, 32, al_map_rgb(255, 255, 255),
                                                                    0, 0, i*64 + 4000 - map_ajustment, vector_platform[3]->y + 48, 2.0, 2.0, 0.0, 0);
                        }
                    }
...
void init_stuff() {
    ...
    all_background = al_load_bitmap("background_cyber.png");
    ...
    main_character = al_load_bitmap("sprite_wgun3.png");
    must_init(main_character, "main character sprite");
       
    bullet_art = al_load_bitmap("bullet.png");
...
char update_position(){			
    ...
    if(player_1->hp == 0){
        update_bullets(player_1->gun, 0, SCREEN_X);
        return -1;
    }
